...
import useStyles from './ProductTransactionStyles'
...
function ProductTransaction() {
	...
	const getRegistrationTransactions = () => {
		setLoading(true);
		const baseUrl = process.env.REACT_APP_API_END_POINT;
		axios.get(baseUrl+`/my_smart_pay_package/vip/${recordsPerPage}?page=${currentPage}`)
		.then(function (res) {
			setData(res.data['smart_pay_packages']['data']);
			setTotalPages(res.data['smart_pay_packages']['last_page']);
		})
		.catch(function (err) {
			console.log('error getting vip package transactions', err)
		})
    ...
    return (
        <div>
            <Box className="pageHeader">
				<Typography className="mainHeading" gutterBottom component="h1" variant="h4">VIP Transactions</Typography>
				<RouterBreadcrumbs />
			</Box>
			{loading ?
